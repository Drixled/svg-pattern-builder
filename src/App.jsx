    ...
    for (let x = spacing; x < canvasSize.width; x += spacing) {
      for (let y = spacing; y < canvasSize.height; y += spacing) {
        ...
        switch (opacityPattern) {
          ...
          case "wave":
            opacity *= Math.abs(
              Math.sin((x / 50 + seed * 10) * Math.PI) *
                Math.cos((y / 50 + seed * 10) * Math.PI)
            );
            break;
        }
    ...
    switch (shapeType) {
      ...
      case "triangles":
        ...
        return (
          <polygon
            key={index}
            points={`${shape.x},${shape.y - height / 2} ${
              shape.x - shape.size / 2
            },${shape.y + height / 2} ${shape.x + shape.size / 2},${
              shape.y + height / 2
            }`}
            fill={shapeColor}
            opacity={shape.opacity}
      ...
      <div style={styles.column}>
        ...
        <div style={styles.control}>
          <label style={styles.label}>
            Min Size (px): {minSize.toFixed(1)}px
          </label>
          <input
            type="range"
            min="0.5"
        ...
        <div style={styles.control}>
          <label style={styles.label}>
            Max Size (px): {maxSize.toFixed(1)}px
          </label>
          <input
            type="range"
            min="0.5"
